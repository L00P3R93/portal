<?php
    ...
    if(isset($_POST)){
        ...
        if(!empty($_REQUEST['post_sub'])){
            ...
            {
                $sql2="UPDATE posts SET title='$title',image='$NewImageName',descr='$body' WHERE post_id=$id";
                $r = mysqli_query($dbconn,$sql2)or die(mysqli_error($dbconn));

            }else {
                //Upload product image to database
                $sql1="UPDATE posts SET title='$title',image='$BackgroundNewImageName',descr='$body' WHERE post_id=$id";
            ...
            if(isset($_FILES['ImageFile'])){
                $sql3 = "INSERT INTO posts(title,image,descr) VALUES('$title','$NewImageName','$body')";
                $r = mysqli_query($dbconn, $sql3)or die(mysqli_error($dbconn));
            }else{
                $sql3 = "INSERT INTO posts(title,image,descr) VALUES('$title','$BackgroundNewImageName','$body')";
                $r = mysqli_query($dbconn, $sql3)or die(mysqli_error($dbconn));
            }
        ...
        if(isset($id)){
    		...
    		header("location:../posts.php?id=$id&url=port&upd=success");
        }else{
        	header("location:../posts.php?url=port&ins=success");
        }
