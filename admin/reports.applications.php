...
<?php
  if($_POST){
    ...
    $sql = "SELECT * FROM applications WHERE add_date >= '$startdate' AND add_date <= '$enddate'";
    ...
    while($tab = mysqli_fetch_assoc($query)){
       $table .='
                <tr>
                    <td>'.$tab['appl_code'].'</td>
                    <td>'.$tab['course_code'].'</td>
                    <td>'.$tab['course_name'].'</td>
                    <td>'.$tab['user_name'].'</td>
                    <td>'.$tab['reg_no'].'</td>
                    <td>'.$tab['add_date'].'</td>
                </tr>';
...
<body data-spy="scroll" data-target=".onpage-navigation" data-offset="60">
    ...
    <?php include 'controllers/navigation/first-navigation.php' ?>
        <div class="container" id="planner">
            <h3 class="text-center">Application Reports</h3>
            <hr>
            <br>
            <form action="reports.applications.php" method="post" enctyp="multipart/form-data">
                <div class="row">
